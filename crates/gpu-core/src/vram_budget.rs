...
impl VramBudget {
    ...
    pub fn from_available_vram(vram_mb: u32) -> Self {
        let (max_particles, hash_grid_capacity, render_width, render_height) =
...
fn budget_for_tier(vram_mb: u32) -> (u32, u32, u32, u32) {
    if vram_mb >= 20_000 {
        // 24 GB+ tier (RTX 4090, A6000, etc.)
        return (2_000_000, 4_194_304, 1920, 1080);
    }
...
fn budget_for_lower_tier(vram_mb: u32) -> (u32, u32, u32, u32) {
    if vram_mb >= 10_000 {
        // 12 GB tier (RTX 4070 Ti, etc.)
        (1_000_000, 2_097_152, 1280, 720)
    } else if vram_mb >= 7_000 {
        // 8 GB tier (RTX 4060, etc.)
        (500_000, 1_048_576, 1280, 720)
    } else {
        // 6 GB or less
        (200_000, 524_288, 960, 540)
    }
}
